...
class Peer(object):
    def __init__(self, number_of_pieces, ip, port=6969):
        self.last_call = 0.0
    ...
    def request_block(self, piece_index, block_offset, block_length):
        request = message.Request(piece_index, block_offset, block_length).to_bytes()
        self.send_to_peer(request)
    ...
    def handle_request(self, request):
        ...
        if self.is_interested() and self.is_unchoked():
            block = self.pieces_manager.get_block(request.piece_index, request.block_offset, request.block_length)
            if block:
                piece = message.Piece(request.piece_index, request.block_offset, request.block_length, block).to_bytes()
                self.send_to_peer(piece)
                logging.info(f"Sent piece index {request.piece_index} to peer : {self.ip}")
